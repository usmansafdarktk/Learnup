	...
	const testimonialsList = [
		{
			...
			name: "Étienne",
			content:
				"J'ai lancé mon premier business sur Learneup par la vente d'ebook grâce à multitude de personnes connectées sur la plateforme chaque jour j'ai pu générer mes premiers milliers d'euros sans publicités c'est juste incroyable",
		},
		{
			profile: "/user-2.jpg",
			name: "Marianne",
			content:
				"Je suis coach formatrice en développement personnel, un amis m’a parlé de Learneup comme étant le canal idéal pour proposer mes services, après quelques hésitations j’ai franchi le pas et je peux vous dire que ça a été la meilleure décision que j’ai prise de l’année.",
		},
		{
			profile: "/user-3.jpg",
			name: "Yvan",
			content:
				"Learneup est une vraie révolution dans le domaine entrepreneurial, je ne m’y fais toujours pas juste un abonnement qui ne coûte rien et vous pouvez toucher vos premiers clients sans publicités…",
		},
		{
			profile: "/user-4.jpg",
			name: "Rodrigue ",
			content:
				"Je suis développeur web et grâce à Learneup j’arrive aujourd’hui à vivre de ma passion. Merci aux fondateurs c’est une vraie réussite",
		}
	];

	return (
		<div className="section container space-y-10">
			<SectionHeader label="Que disent nos clients ?" className="dark:text-gray-100" />

			<div className="pb-5 px-5">
				<div className="embla" ref={emblaRef}>
					<div className="embla__container space-x-4 flex">
						{testimonialsList.map((item, idx) => (
							<div
								key={idx}
								className={cn(
									"embla__slide bg-white dark:bg-gray-700/50 rounded-xl border dark:border-gray-600 shadow-md flex flex-col max-w-xl",
									item.name === "Irina" && "mr-6"
								)}
							>
								<div className="p-4">
									<svg
										className="w-9 h-9 text-gray-800 dark:text-gray-400"
										viewBox="0 0 35 35"
										fill="none"
								...
								<div className="">
									<div className="flex items-center gap-x-4 p-4 bg-primary-100 dark:bg-gray-600">
										...
										<div>
											<span className="block text-gray-800 dark:text-gray-100 font-heading-semibold">{item.name}</span>
										</div>
									</div>
									<blockquote>
										<p className="text-gray-800 lg:text-lg px-4 py-1 dark:text-gray-200">{item.content}</p>
									</blockquote>
								</div>
							</div>
						))}
